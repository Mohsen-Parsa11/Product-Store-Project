    ...
    <div>
      <Container>
        <div className="w-full py-4 my-5 shadow border border-gray-300 px-3 lg:px-10 flex flex-col md:flex-row">
          ...
          <p className="text-gray-600 text-2xl line-clamp-1">Title: {data?.title}</p>
          <p className="text-gray-500 line-clamp-3 my-1 ">Description: {data?.description}</p>
          <p className="text-gray-400 text-lg">Rating: {data?.rating}</p>
          <p className="text-gray-400 text-lg">Price: ${data?.price}</p>
          <AddToCart id={id} />
          </div>
        </div>
      </Container>
